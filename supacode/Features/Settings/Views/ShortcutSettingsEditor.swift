...
struct ShortcutSettingsEditor: View {
  ...
  var body: some View {
    Group {
      if let commandID {
        if let command = editableCommands.first(where: { $0.id == commandID }) {
          row(for: command)
        }
      } else {
        shortcutTable
    ...
    .task(id: store.shortcutNavigationTargetCommandID) {
      guard self.commandID == nil, let commandID = store.shortcutNavigationTargetCommandID else { return }
      ...
      HStack(alignment: .center, spacing: 12) {
        HStack(spacing: 5) {
          Text(command.title)
            .lineLimit(1)
            .truncationMode(.tail)
          if command.id == AppShortcuts.CommandID.toggleAgentIsland {
            Image(systemName: "globe")
              .foregroundStyle(.secondary)
              .help(Self.globalShortcutHelp)
              .accessibilityLabel("System-wide shortcut")
          }
        }
        .frame(minWidth: ShortcutTableLayout.commandColumnMinWidth, maxWidth: .infinity, alignment: .leading)
        .layoutPriority(1)
