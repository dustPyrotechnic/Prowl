...
private func handoffCommandItems(_ repositories: RepositoriesFeature.State) -> [CommandPaletteItem] {
  ...
  return [
    CommandPaletteItem(
      ...
      title: String(localized: "Hand Off…"),
      subtitle: String(localized: "Choose an agent to take over this task"),
      kind: .handOff,
      category: .terminal,
      defaultSuggestion: false,
      keywords: ["Hand Off", "handoff", "hand off", "switch agent", "takeover", "agents", "交接", "切换 Agent"]
    )
  ]
}
