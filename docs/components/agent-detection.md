...
2. **Screen heuristics.** Claude detection consumes the full active screen (bounded
   ...
   When `Jump to bottom (click) ↓` appears just above that composer, Prowl treats the
   screen as a viewer and retains the last known state. Return to the bottom to refresh
