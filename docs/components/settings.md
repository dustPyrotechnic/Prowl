...
| Section | Controls |
|-----|----------|
| **General** | Application language (System Default, 简体中文, English; takes effect on next launch), appearance (system/light/dark), default app for opening worktrees, diff tool, confirm-before-quit, default view mode, window chrome tint, automatic repository icon detection, toolbar buttons (Run / Open-in-editor), dim unfocused splits. |
